...
function sl_enqueue_scripts() {
	wp_enqueue_script( 'simple-likes-public-js', get_template_directory_uri() . '/assets/js/components/like.js', array( 'jquery' ), '0.5', false );
	wp_localize_script( 'simple-likes-public-js', 'simpleLikes', array(
		'ajaxurl' => admin_url( 'admin-ajax.php' ),
		'like' => __( 'Like', 'get-web-advice' ),
		'unlike' => __( 'Unlike', 'get-web-advice' )
	) ); 
}
...
function process_simple_like() {
	...
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( __( 'Not permitted', 'get-web-advice' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = __( 'Unlike', 'get-web-advice' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = __( 'Like', 'get-web-advice' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	$like_text = __( 'Like', 'get-web-advice' );
